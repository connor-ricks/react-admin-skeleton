...
export const COOKIES_REFRESH_TOKEN_AGE = 7 * 24 * 60 * 60; // 7 Day Refresh Expiration.
 ...
 */
export async function setCookiesAccessToken(token, shouldRemember) {
  const store = await cookies();
  store.set(COOKIES_ACCESS_TOKEN_KEY, token, {
    ...
    sameSite: 'strict',
    maxAge: shouldRemember ? COOKIES_ACCESS_TOKEN_AGE : null,
    path: '/',
  });
}
 ...
 * @returns {Promise<void>}
 */
export async function setCookiesRefreshToken(token, shouldRemember) {
  const store = await cookies();
  store.set(COOKIES_REFRESH_TOKEN_KEY, token, {
    httpOnly: true,
    secure: false, // true, TODO: <Connor> This should be secure but it doesn't work on Safari unless it is false.
    sameSite: 'strict',
    maxAge: shouldRemember ? COOKIES_REFRESH_TOKEN_AGE : null,
    path: '/',
  });
}
